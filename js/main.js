...
minus.addEventListener('click', function(){
    ...
    if(num > 0){
      let minusRandom = Number(prompt("Введите сколько хотите отнять очков"))
      if(num - minusRandom  < 0){
    alert("Нельзя отнять так чтобы было меньше нуля")
    }else{
      num = num - minusRandom
      console.log("Minus", num)
      alert("Minus ", minusRandom)
      render()
          } 
    }
